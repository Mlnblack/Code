...
parser.add_argument('--dataroot', default='F:/实验/SO-HandNet-master/data/ICVL/process_output/',help='path to images & laser point clouds')
    ...
    for epoch in range(30):
        ...
        for i, data in enumerate(tqdm(trainloader,0)):
            ...
            estimation_joins = estimation_joins.view(estimation_joins.size(0), 1,
                                                         estimation_joins.size(1))  # [ ,1,48]
            outputs_xyz = estimation_joins.cpu().view(gt_xyz.shape)

            # re-compute
            outputs_xyz = outputs_xyz.detach().numpy()  # [8,3,16]
            gt_xyz = gt_xyz.detach().numpy()  # [8,3,16]
            ...
            for ii in range(len_list):
                output_xyz = outputs_xyz[ii, :, :]  # (3,16)
                g_xyz = gt_xyz[ii, :, :]  # (3,16)
                out_xyz = np.zeros_like(output_xyz)  # (3,16)
                ...
                for jj in range(output_xyz.shape[1]):
                    ...
                    gxyz[:, jj] = ggxyz
                out_xyz = out_xyz * v_length  # (3,16)
                out_xyz = np.dot(out_xyz.T, np.linalg.inv(v_rotate.T))  # (16,3)
                out_xyz = out_xyz.T  # (3,16)
            ...
            diff = torch.pow(output_xyzs - gt_xyzs, 2)  # [8,3,16]
            diff_sum = torch.sum(diff, 1)  # [8,16]
            diff_sum_sqrt = torch.sqrt(diff_sum)  # [8,16]
